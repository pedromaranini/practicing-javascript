...
const downNumbers = crazyNumbers.reduce((accumulator, item) => accumulator + (item < 501), 0);
...
const afterMovies = tarantinoMovies.filter((movie) => movie.release < 2000 )
...
const nameSeries = tvShows.map((item) => item.name)
...
const newCart = cart.map((item, index) => `Nome ${index}: ${item.name}`)
